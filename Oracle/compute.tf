data "template_file" "user_data" {
  template = file("web.sh")
}

resource "oci_core_instance" "WebServer1" {
  availability_domain = "aSHn:eu-amsterdam-1-AD-1"
  ...
  create_vnic_details {
    subnet_id = oci_core_subnet.WebTier.id
    nsg_ids   = [oci_core_network_security_group.WebSG.id, oci_core_network_security_group.SshSG.id]
    display_name = "Web-Server-1"
  }
  source_details {
    source_type = "image"
